-- ============================================================
-- Smart Analytics v1:
--   1. dead_stock       — active styles với stock cao nhưng bán rất thấp
--   2. reorder_urgency  — dự báo số ngày còn hàng theo vận tốc bán
--   3. rpv              — Revenue Per View theo từng category
--   4. return_revenue   — doanh thu bị mất do trả hàng theo category
--   5. channel_perf     — hiệu suất theo kênh bán
-- Tất cả đều bổ sung vào dashboard_stats() trong một round trip.
-- ============================================================
...
begin
  ...
  -- ── 1. DEAD STOCK ALERT ────────────────────────────────────────
  -- Active styles that have been live >= 30 days but units_sold = 0,
  -- OR styles where views > 200 but conversion (units_sold/views) < 1%.
  -- Score: higher = more urgent.  Cap at 20 rows.
  dead_stock as (
    select jsonb_agg(d order by d.score desc) as rows from (
      select
        s.id,
        s.name,
        s.style_code,
        ...
        -- urgency score: stock × days_live / max(units_sold,1)
        -- penalise dead items with lots of stock sitting a long time
        (coalesce(sum(v.stock), 0) * extract(day from now() - s.created_at)
          / greatest(s.units_sold, 1))::int as score,
        -- alert level
        case
          when s.units_sold = 0 and coalesce(sum(v.stock), 0) > 10
               and extract(day from now() - s.created_at) > 60 then 'critical'
          when s.units_sold = 0 and extract(day from now() - s.created_at) > 30 then 'warning'
          when s.views > 200
      ...
      where s.status = 'active'
        and (
          -- zero-sellers after 30 days
          (s.units_sold = 0 and s.created_at < now() - interval '30 days')
          -- or poor conversion with decent traffic
  ...
  -- ── 2. REORDER URGENCY ─────────────────────────────────────────
  -- For each active style: compute sell velocity (units_sold / days_live),
  -- then estimate days_until_stockout = total_stock / velocity.
  -- Only return styles where stockout is predicted within 30 days.
  reorder_urgency as (
    select jsonb_agg(r order by r.days_until_oos asc) as rows from (
      select
        s.id,
        s.name,
        s.style_code,
        ...
        s.units_sold,
        coalesce(sum(v.stock), 0)::int as total_stock,
        -- sell velocity: units per day since created
        case
        ...
        case
          ...
          else null  -- can't estimate (no sales yet or velocity = 0)
      ...
      left join public.variants v on v.style_id = s.id
      where s.status = 'active'
      group by s.id, s.name, s.style_code, s.images, s.units_sold, s.price, cat.label, s.created_at
      having
        -- must have some stock left AND some sales velocity
  ...
  -- For each category: total paid revenue / total views → RPV
  -- Shows which categories turn traffic into money most efficiently.
  rpv_by_category as (
    select jsonb_agg(jsonb_build_object(
      'id',           cat.id,
      'label',        cat.label,
      'revenue',      coalesce(sum(pi.qty * pi.price), 0)::bigint,
      'views',        coalesce(sum(s.views), 0)::bigint,
      'units_sold',   coalesce(sum(s.units_sold), 0)::bigint,
      'rpv',          case
                        when coalesce(sum(s.views), 0) = 0 then 0
                        else round(coalesce(sum(pi.qty * pi.price), 0)::numeric
                                   / coalesce(sum(s.views), 0))::bigint
                      end
    ) order by (case when coalesce(sum(s.views), 0) = 0 then 0
                     else coalesce(sum(pi.qty * pi.price), 0)::numeric / coalesce(sum(s.views), 0)
                end) desc) as rows
    from public.categories cat
  ...
  -- For each category: how much revenue was lost to returns.
  -- return_value = sum of returned order items value per category.
  return_revenue_by_cat as (
    select jsonb_agg(jsonb_build_object(
      'id',           cat.id,
      'label',        cat.label,
      'paid_revenue', coalesce(paid_rev.v, 0)::bigint,
      'return_value', coalesce(ret_rev.v, 0)::bigint,
      'return_pct',   case
                        when coalesce(paid_rev.v, 0) + coalesce(ret_rev.v, 0) = 0 then 0
                        else round(
                          coalesce(ret_rev.v, 0)::numeric * 100
                          / (coalesce(paid_rev.v, 0) + coalesce(ret_rev.v, 0))
                        )::int
                      end
    ) order by coalesce(ret_rev.v, 0) desc) as rows
    from public.categories cat
    left join (
      select s.category_id, coalesce(sum(pi2.qty * pi2.price), 0)::bigint as v
      from public.styles s
      left join paid_items pi2 on pi2.style_id = s.id
      group by s.category_id
    ) paid_rev on paid_rev.category_id = cat.id
    left join (
      select s.category_id, coalesce(sum(ri2.qty * ri2.price), 0)::bigint as v
      from public.styles s
      left join returned_items ri2 on ri2.style_id = s.id
      group by s.category_id
    ) ret_rev on ret_rev.category_id = cat.id
  ),
